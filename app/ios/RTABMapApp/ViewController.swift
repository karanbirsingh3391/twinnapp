...
class ViewController: GLKViewController, ARSessionDelegate, RTABMapObserver, UIPickerViewDataSource, UIPickerViewDelegate, CLLocationManagerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if(passedIndex != nil){
            let timer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(openExistingDatabase), userInfo: nil, repeats: false)
        }
       
    }
    
    @objc func openExistingDatabase()
    {
        updateDatabases();
        if databases.isEmpty {
            return
        }
        self.openDatabase(fileUrl: self.databases[self.passedIndex!])
    }
